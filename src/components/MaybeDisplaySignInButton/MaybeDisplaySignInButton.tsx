...
const MaybeDisplaySignInButton = ({ frameContext, jwt, isAuthenticated, onFarcasterSignIn }: { frameContext: FrameContext | null, jwt: string, isAuthenticated: boolean, onFarcasterSignIn: (data: UseSignInData) => void }) => {
    ...
    const framesLogin = () => {
        FramesSDK.actions.signIn({ nonce }).then(({ signature, message }) => {
            if (frameContext && signature && message) {
                onFarcasterSignIn({ nonce, signature: signature as `0x${string}`, message, fid: frameContext.user.fid, username: frameContext.user.username } as any);
            }
        });
    };
